	...
	public void onClick(View v) {
		...
		if (v == changepwd_sub) {
			...
			if (changepwd_old.getText().toString().trim()
				...
				ToastUtil.showCenterShort(context, "新设密码和输入的原始密码一致！");
				return;
			}
			if (!changepwd_re.getText().toString().trim()
					.equals(changepwd_new.getText().toString().trim())) {
				ToastUtil.showCenterShort(context, "新设密码和确认密码不一致！");
				return;
			}
