...
//		price="0.01";//测试使用
		...
		if (!TextUtils.isEmpty(price)) {
			...
			try {
				...
				e.printStackTrace();
				ToastUtil.showCenterShort(context, "充值金额错误！");
				return;
			}
			if (money > 0) {

			} else {
				ToastUtil.showCenterShort(context, "充值金额不能小于0！");
				return;
			}

		} else {
			ToastUtil.showCenterShort(context, "充值金额不能为空！");
			return;
		}
		// 订单
		...
		try {
			...
			e.printStackTrace();
			ToastUtil.showCenterShort(context, "RSA签名错误！");
			return;
		}
		// 完整的符合支付宝参数规范的订单信息
