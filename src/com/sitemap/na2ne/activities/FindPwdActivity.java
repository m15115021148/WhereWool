 ...
 * 找回密码页面
 * 
 * @author zhang create at 2015年1月11日 下午2:02:07
 */
public class FindPwdActivity extends Activity implements
		OnClickListener {
	public static FindPwdActivity context;//本类
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (MyApplication.isLogin<0) {
			Intent intent=new Intent(FindPwdActivity.this,HomePageActivity.class);
...
//						send_code.setTextColor(Color.rgb(255,255,255));
				...
				if (msg.arg1 ==three) {
					...
					switch (status) {
					case zero:
						Intent intent =new Intent(FindPwdActivity.this,FindPwdSureActivity.class);
				...
				send_code.setClickable(false);
				 timer5= new Timer();
					timer5.schedule(new TimerTask() {
						...
						@Override
						public void run() {
								handler.sendEmptyMessage(nine);
						}
							
						},10, 1000);
				if (MyApplication.getNetObject().isNetConnected()) {
				String url = WebUrlConfig.getCodes(register_phone_num_ed.getText().toString().trim());
				http.sendGet(two,url);
				} else {
...
//			}
	...
	public boolean isMobileNO(String mobiles) {
		Pattern p = Pattern.compile("^((13[0-9])|170|(15[^4,\\D])|(18[0-9]))\\d{8}$");
